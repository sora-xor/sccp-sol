...
fn set_inbound_domain_paused(
    ...
    let bit = 1u64.checked_shl(source_domain).ok_or(SccpError::DomainUnsupported)?;
    if paused {
        cfg.inbound_paused_mask |= bit;
    } else {
        cfg.inbound_paused_mask &= !bit;
    }
    write_borsh::<Config>(config_acc, &cfg)?;
...
fn invalidate_inbound_message(
    ...
    let (expected, bump) = inbound_marker_pda(program_id, source_domain, &message_id);
    if *marker_acc.key != expected {
        return Err(SccpError::InvalidPda.into());
    }

    ensure_marker_account(program_id, signer, marker_acc, source_domain, &message_id, bump)?;

    let mut m = read_borsh::<InboundMarker>(marker_acc)?;
    // If already processed, invalidation can't undo the mint; treat as an error to avoid false expectations.
    if matches!(m.status, InboundStatus::Processed) {
        return Err(SccpError::InboundAlreadyProcessed.into());
    }
    m.status = InboundStatus::Invalidated;
...
fn clear_invalidated_inbound_message(
    ...
    let mut m = read_borsh::<InboundMarker>(marker_acc)?;
    if matches!(m.status, InboundStatus::Invalidated) {
        m.status = InboundStatus::None;
        write_borsh::<InboundMarker>(marker_acc, &m)?;
    }
    Ok(())
}
...
fn mint_from_proof(
    ...
    if marker_acc.owner == program_id {
        let m = read_borsh::<InboundMarker>(marker_acc)?;
        match m.status {
...
fn create_pda_account<'a>(
    ...
    if new_acc.owner == owner {
        return Err(SccpError::InvalidOwner.into());
    }
    let rent = Rent::get()?;
    let lamports = rent.minimum_balance(space);
    let ix = system_instruction::create_account(payer.key, new_acc.key, lamports, space as u64, owner);
